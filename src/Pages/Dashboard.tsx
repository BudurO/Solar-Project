        ...
        <div className="flex flex-col h-screen bg-gray-100">
            ...
            <div className="flex-1 flex flex-wrap">
                ...
                <div className="flex-1 p-4 w-full md:w-1/2">
                    <div className=" bg-white pb-4 shadow rounded-lg">
                        <table className="w-full table-auto text-sm mb-4 ">
                            <thead className='bg-yellow-200 rounded-t-lg'>
                                <tr className="text-sm leading-normal">
                                    ...
                                    <th className="py-2 px-4 bg-grey-lightest font-bold uppercase text-sm text-grey-light border-b border-grey-light">House address</th>
                                    <th className="py-2 px-4 bg-grey-lightest font-bold uppercase text-sm text-grey-light border-b border-grey-light">Status</th>
                                    <th className="py-2 px-4 bg-grey-lightest font-bold uppercase text-sm text-grey-light border-b border-grey-light">Phone</th>
                                    <th className="py-2 px-4 bg-grey-lightest font-bold uppercase text-sm text-grey-light border-b border-grey-light">Area of roof</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                {Info.map((item)=>{
                                    return(
                                        <tr className="hover:bg-grey-lighter" key={item.id}>
                                            <td className="py-2 px-4 border-b border-grey-light border border-dashed  rounded-full h-10 w-10">#{cunt++}</td>
                                            <td className="py-2 px-4 border-b border-grey-light border-r">{item.Locaiton.placeName}</td>
                                            <td className="py-2 px-4 border-b border-grey-light border-r">{item.State}</td>
                                            <td className="py-2 px-4 border-b border-grey-light border-r">{item.Number}</td>
                                            <td className="py-2 px-4 border-b border-grey-light border-r">{item.Area}</td>
                                            <td className="py-2 px-4 border-b border-grey-light">
                                                <button onClick={()=> deleteRent(String(item.id))} className="text-white bg-red-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center  dark:bg-red-600 dark:hover:bg-red-700">
